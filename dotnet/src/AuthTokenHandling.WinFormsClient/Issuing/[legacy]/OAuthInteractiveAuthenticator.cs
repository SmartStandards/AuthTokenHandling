...
namespace Security.AccessTokenHandling {
  ...
  [Obsolete("Please use one of these instead: 'EmbeddedBrowserOAuthIssuer' (also supports hidden mode) / 'ExternalBrowserOAuthIssuer'")]
  public class OAuthInteractiveAuthenticator {
    ...
    public OAuthInteractiveAuthenticator(
      ...
      _Base = new OAuthBackgroundAuthenticator(
        clientId, clientSecret, redirectUrl, authorizeUrl, retrievalUrl, claimApprovalHandler
      );
    ...
    public bool TryBrowserAuthViaCodeGrand(IWin32Window windowOwner,
      ...
      using (var dlg = new AuthForm()) {
        dlg.CurrentUrl = $"{_AuthorizeUrl}response_type=code&redirect_uri={returnUrl}&state={state}&scope={scopeToRequest}&login_hint={loginHint}&client_id={_ClientId}{additionalQueryParameters}";
        ...
        string code = null;
        if (dlg.CurrentUrl.StartsWith(returnUrl, StringComparison.InvariantCultureIgnoreCase)) {
          code = this.PickFromUrl(dlg.CurrentUrl, "code");
        }
    ...
    public bool TryBrowserAuthViaImplicitGrand(IWin32Window windowOwner,
      ...
      using (var dlg = new AuthForm()) {
        dlg.CurrentUrl = $"{_AuthorizeUrl}response_type=token&redirect_uri={returnUrl}&state={state}&scope={scopeToRequest}&login_hint={loginHint}&client_id={_ClientId}{additionalQueryParameters}";
        dlg.ReturnOn = returnUrl;
        ...
        string token = null;
        if (dlg.CurrentUrl.StartsWith(returnUrl, StringComparison.InvariantCultureIgnoreCase)) {
          token = this.PickFromUrl(dlg.CurrentUrl, "access_token");
          refreshToken = this.PickFromUrl(dlg.CurrentUrl, "refresh_token");
          idToken = this.PickFromUrl(dlg.CurrentUrl, "id_token");
          //? = this.PickFromUrl(dlg.Url, "token_type");
          //? = this.PickFromUrl(dlg.Url, "expires_in");
          string error = this.PickFromUrl(dlg.CurrentUrl, "error");
        }
